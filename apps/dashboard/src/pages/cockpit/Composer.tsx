    ...
    return (
      <div className="ck-composer">
        <div className="ck-new-fields">
          <div>
            <label>Repo · 仓库</label>
            <select data-testid="cockpit-repo-select" value={props.repoId} onChange={(e) => props.onRepoChange(e.target.value)}>
        ...
        <div className="ck-composer-actions">
          <button data-testid="cockpit-start-brainstorm" className="ck-btn primary" disabled={busy || !props.draft.trim()} onClick={props.onBrainstorm}>
  ...
  return (
    <div className="ck-composer">
      ...
      <div className="ck-composer-actions">
        ...
        <button className="ck-btn" disabled={busy} onClick={props.onAsk}>Ask Until Clear · 继续问清楚</button>
      </div>
    </div>
  )
}
